...
maine_buoys <- read_csv("/cloud/project/maine_buoy.csv")
...
## OpenStreetMap chosen because of extensive labelling (helpful when exploring a mapping tool)

leaflet(maine_buoys) %>%
  addTiles() %>%
  addProviderTiles(providers$OpenStreetMap) %>%
  addMarkers(~lon, ~lat, popup = ~labels) %>%
    setView(lng = -69, lat = 43.8, zoom = 7.5)  
